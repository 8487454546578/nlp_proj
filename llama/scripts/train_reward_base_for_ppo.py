...
for epoch in range(EPOCHS):
    ...
    if acc > best_acc:
        best_acc = acc
        torch.save(reward_model.state_dict(), "reward_model_pairwise_best_for_ppo.pt")
        print(f"Saved best model at epoch {epoch+1} with acc {acc:.4f}")

# ==== 加载最佳模型并评估测试集准确率 ====
print("\nEvaluating best model on test set...")
best_model = RewardModel(base_model, freeze_base=True).to(device)
best_model.load_state_dict(torch.load("reward_model_pairwise_best_for_ppo.pt"))
...
plt.xlabel("Epoch")
plt.ylabel("Value")
plt.title("Pairwise Reward Model Training For PPO")
plt.legend()
plt.grid(True)
plt.savefig("rlhf_pic/pairwise_loss_acc.png")
plt.close()
